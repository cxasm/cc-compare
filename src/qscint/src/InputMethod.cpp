...
void QsciScintillaBase::inputMethodEvent(QInputMethodEvent *event)
{
    ...
    if (!event->commitString().isEmpty()) {
        ...
        Sci::Position curpos = sci->CurrentPosition();
        int imeCaretPosDoc = sci->pdoc->GetRelativePositionUTF16(curpos, imeEndToImeCaretU16);

        //20240630发现，这个imeCaretPosDoc有可能返回-1 INVALID_POSITION，即失败。此时要加个保护，否则macOS下面失去光标，而且崩溃
        //即如果失败了，则保持光标当前的位置。
        if(imeCaretPosDoc != -1)
        {

            MoveImeCarets(sci, -curpos + imeCaretPosDoc);
        }
...
{
    int pos = 0; // SendScintilla(SCI_GETCURRENTPOS);
    int line = 0; // SendScintilla(SCI_LINEFROMPOSITION, pos);
