...
{
    //20240306 巨坑：用户反馈替换一个23万行4M的小文件，非常卡顿。发现是在LineLayoutCache::Invalidate函数的循环中要遍历23万次
    //对此竞品后发现人家就没有缓存一个如此大而慢的循环。就是下面这条语句在作祟。这是不对的，模块换行是自动开启的，导致下面缓存
    // 是全文缓存，所以替换中会每次替换都会全部LineLayoutCache::Invalidate导致异常缓慢。排查一天才定位到此。
...
void QsciScintilla::contextMenuEvent(QContextMenuEvent *e)
{
    if (contextMenuNeeded(e->x(), e->y()))
    {
        ...
        contextUserDefineMenuEvent(menu,e);
...
void QsciScintilla::contextUserDefineMenuEvent(QMenu* menu, QContextMenuEvent* e)
{
}
