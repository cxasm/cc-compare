...
!getdllversion "..\x64\Release\CCompare.exe" nnver_
...
!define PRODUCT_NAME "CCompare"
!define COMPANY_NAME "${PRODUCT_NAME}"
!define PROGEXE "CCompare.exe"
!define MULTIUSER_INSTALLMODE_UNINSTALL_REGISTRY_KEY "CCompare"
...
!define MUI_ICON "..\Resources\img\main.ico"
!define MUI_UNICON "..\Resources\img\main.ico"
...
!define MEMENTO_REGISTRY_KEY Software\Microsoft\Windows\CurrentVersion\Uninstall\CCompare
...
Name "CCompare v${VERSION}"
OutFile "CCompare-v${VERSION}-x64-Installer.exe"
...
VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "CCompare"
VIAddVersionKey /LANG=${LANG_ENGLISH} "LegalCopyright" "Copyright 2019-2023"
VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "CCompare v${VERSION} Installer"
...
Function .onInit
	${ifnot} ${UAC_IsInnerInstance}
		!insertmacro CheckSingleInstance "Setup" "Global" "CCompareSetupMutex"
	${endif}
...
Section "CCompare"
	...
	WriteRegStr SHCTX "Software\Microsoft\Windows\CurrentVersion\App Paths\CCompare" "" "$INSTDIR\CCompare.exe"
	...
	IfErrors +3 0
	IntFmt $0 "0x%08X" $0
	WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\CCompare" "EstimatedSize" "$0"
SectionEnd

# -----------------------------------------------

${MementoSection} "Desktop Shortcut" SEC_DESKTOP_SHORTCUT
	CreateShortCut "$DESKTOP\CCompare.lnk" "$INSTDIR\CCompare.exe"
${MementoSectionEnd}

Section "-Remove Desktop Shortcut" SEC_REMOVE_DESKTOP_SHORTCUT
	Delete "$DESKTOP\CCompare.lnk"
SectionEnd

# -----------------------------------------------

${MementoSection} "Start Menu Shortcut" SEC_START_MENU_SHORTCUT
	CreateShortCut "$SMPROGRAMS\CCompare.lnk" "$INSTDIR\CCompare.exe"
${MementoSectionEnd}

Section "-Start Menu Shortcut" SEC_REMOVE_START_MENU_SHORTCUT
	Delete "$SMPROGRAMS\CCompare.lnk"
SectionEnd
...
${MementoSection} "Context Menu" SEC_CONTEXT_MENU
	SetRegView 64
	WriteRegStr SHCTX "Software\Classes\*\shell\CC-L选为对比的左边文件" "Icon" "$INSTDIR\CCompare.exe"
	WriteRegStr SHCTX "Software\Classes\*\shell\CC-L选为对比的左边文件\command" "" "$\"$INSTDIR\CCompare.exe$\" -left $\"%1$\""
	WriteRegStr SHCTX "Software\Classes\*\shell\CC-R选为对比的右边文件" "Icon" "$INSTDIR\CCompare.exe"
	WriteRegStr SHCTX "Software\Classes\*\shell\CC-R选为对比的右边文件\command" "" "$\"$INSTDIR\CCompare.exe$\" -right $\"%1$\""

    WriteRegStr SHCTX "Software\Classes\Directory\shell\CC-L选为对比的左边目录" "Icon" "$INSTDIR\CCompare.exe"
	WriteRegStr SHCTX "Software\Classes\Directory\shell\CC-L选为对比的左边目录\command" "" "$\"$INSTDIR\CCompare.exe$\" -leftd $\"%1$\""
	WriteRegStr SHCTX "Software\Classes\Directory\shell\CC-R选为对比的右边目录" "Icon" "$INSTDIR\CCompare.exe"
	WriteRegStr SHCTX "Software\Classes\Directory\shell\CC-R选为对比的右边目录\command" "" "$\"$INSTDIR\CCompare.exe$\" -rightd $\"%1$\""
${MementoSectionEnd}

Section "-Context Menu" SEC_REMOVE_CONTEXT_MENU
	SetRegView 64

	DeleteRegKey SHCTX "Software\Classes\*\shell\CC-L选为对比的左边文件"
    DeleteRegKey SHCTX "Software\Classes\*\shell\CC-R选为对比的右边文件"
    DeleteRegKey SHCTX "Software\Classes\Directory\shell\CC-L选为对比的左边目录"
    DeleteRegKey SHCTX "Software\Classes\Directory\shell\CC-R选为对比的右边目录"
SectionEnd
...
Section "Uninstall"
	...
	DeleteRegKey SHCTX "Software\Microsoft\Windows\CurrentVersion\App Paths\CCompare.exe" "" "$INSTDIR\CCompare.exe"
