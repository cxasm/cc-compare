...
m_hasHighlight(false), m_mediator(nullptr), m_copyNeedReplace(false)
{
...
{
	int length = this->SendScintilla(SCI_GETLENGTH);
	...
	this->SendScintilla(SCI_GETTEXT, length+1, contents);

	QString contentsStr(contents);
	...
	int lineNums = (lines.size() > externLineInfo->size()) ? externLineInfo->size() : lines.size();
	...
	for (int i = 0; i < lineNums; ++i)
	{
		...
		{
			QString bt = lines.at(i);

			//按照\n来分割，如果是windows下行尾是\r\n，则必然多一个\r，要去掉该符号
			//行尾以blockUserData->m_lineEndType为确定，所以去掉多余的\r字符
			if (bt.endsWith("\r"))
			{
				bt = bt.left(bt.size()-1);
			}

			savefun(bt, blockUserData->m_lineEndType);
		}
	}
...
void QsciDisplayWindow::contextUserDefineMenuEvent(QMenu* menu, QContextMenuEvent* e)
{
	//QAction* action;
	if (menu != nullptr)
	{
		if (m_copyNeedReplace)
		{
			QAction* pCopy = menu->findChild<QAction*>("copy");
			if (pCopy != nullptr)
			{
				//menu->removeAction(pCopy);
				//menu->addAction(tr("&Copy"), this, &QsciDisplayWindow::on_copy); 
				QAction* pFormatCopy = new QAction(tr("Copy (without formatting)"),menu);
				connect(pFormatCopy, &QAction::triggered, this, &QsciDisplayWindow::on_copy);
				menu->insertAction(pCopy, pFormatCopy);
			}
			QAction* pSelAll = menu->findChild<QAction*>("Select All");
			if (pSelAll != nullptr)
			{
				QAction* pullOpen = new QAction(tr("pull open"), menu);
				connect(pullOpen, &QAction::triggered, this, &QsciDisplayWindow::sign_pullOpen);
				menu->insertAction(pSelAll, pullOpen);
		}
			else
			{
				menu->addAction(tr("pull open"), this, &QsciDisplayWindow::sign_pullOpen);
			}
		}
...
//选择拷贝中，去掉对齐的空白行
void QsciDisplayWindow::on_copy()
{
	...
	if (lineList.size() > 1)
	{
		...
		{
			int markMask = this->SendScintilla(SCI_MARKERGET, i);
			if ((markMask & STYLE_PAD_OR_LASTPAD_BIT_MASK) != 0)
			{
				int index = i - lineFrom;
...
//20230308加个老注释：很早之前做ccompare时，发现对比中输入中文时，容易把取消或中文的中间英文输入，导致对比紊乱。
//于是对输入没有彻底完成的情况，即preeditString有内容的情况下，直接返回。
//20230303升级qscint3.14版本后，好像官方已经解决该问题了，这个函数可以取消。暂时把注释留在这里。
