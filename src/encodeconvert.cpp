...
const int ITEM_CAN_TRANS = Qt::UserRole + 2;

EncodeConvert::EncodeConvert(QWidget *parent): QWidget(parent), m_commitCmpFileNums(0), m_finishCmpFileNums(0), m_menu(nullptr), m_workStatus(0)
{
	ui.setupUi(this);
	...
	connect(ui.extComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &EncodeConvert::on_changeExt);

	connect(ui.showConvertible, &QCheckBox::stateChanged, this, &EncodeConvert::on_showOnlyConvertibleItem);

	//第三项是以.h|.cpp为例子的一个项目。是默认存在的。在20231014中新增。
...
//把所有不能转换的项目：隐藏或显示
void EncodeConvert::on_showOnlyConvertibleItem(int state)
{
	if (m_workStatus != 0)
	{
		return;
	...
	while (*it) {
		...
		else if (RC_FILE == (*it)->type())
		{
			if ((*it)->data(0, ITEM_CAN_TRANS).toInt() != 1)
			{
				(*it)->setHidden((state == Qt::Checked));
			}
		}
		++it;
	}
...
//0 只识别已知类型 1 所以类型，其余：自定义类型
void EncodeConvert::on_changeExt(int index)
{
	if (m_workStatus != 0)
	{
		return;
	...
	//切换了当前的文件后缀处理方式。重新加载文件夹
	if(!m_fileDirPath.isEmpty())
	{
		ui.treeWidget->clear();

		m_fileAttris.clear();

		ui.logTextBrowser->append(tr("Switching file types, rescanning files ..."));

		m_workStatus = 1;

		loadDir(m_fileDirPath);

		scanFileCode();

		m_workStatus = 0;

		on_showOnlyConvertibleItem(ui.showConvertible->checkState());
...
//index:0,只处理已知类型的文件; 1:强行处理所有文件 2:特定的文件后缀类型
bool EncodeConvert::isSupportExt(int index, QString ext)
{
	...
	{
		//第0 个是处理已知类型 //第1个时处理所有类型 //第2个是处理.h|.cpp类型。
		//m_supportFileExt 中的第0个元素，就是对应index==2的 .h|.cpp类型。
		//所以这里有个index定位的偏移
...
void EncodeConvert::slot_itemClicked(QTreeWidgetItem* item, int /*column*/)
{
	if ((item != nullptr) && (Qt::RightButton == QGuiApplication::mouseButtons()))
	{
		...
		if (m_menu == nullptr)
		{
			...
			m_menu->addAction(tr("Delete Select items..."), this, &EncodeConvert::on_delSelLine);
		}
...
void EncodeConvert::on_delSelLine()
{
	//实际是删除当前选中的行
	...
	{
		QTreeWidgetItem* pItem = itemList.at(i);
		...
		fileAttriNode fNode;
		fNode.relativePath = pItem->data(0, Qt::ToolTipRole).toString();

		int index = m_fileAttris.indexOf(fNode);

		if (index != -1)
		{
			m_fileAttris.removeAt(index);
...
//Show only convertible files 只显示可转换文件
...
int EncodeConvert::allfile(QTreeWidgetItem* root_, QString path_)
{
	...
	while (!dirsList.isEmpty())
	{
		...
		for (int i = 0; i != folder_list.size(); ++i)         //自动递归添加各目录到上一级目录
		{
			...
			QTreeWidgetItem* childroot = new QTreeWidgetItem(QStringList() << name, RC_DIR);
		...
		{  //将当前目录中所有文件添加到treewidget中
			...
			QTreeWidgetItem* child = new QTreeWidgetItem(QStringList() << name2, RC_FILE);
			child->setIcon(0, QIcon(":/Resources/img/point.png"));

			child->setText(1, tranFileSize(fileInfo.size())/*CompareDirs::getFileSizeFormat(fileInfo.size())*/);
...
{
	ui.logTextBrowser->append(tr("Loading scan file, please wait ..."));
	...
	return commitTask([](EncodeThreadParameter_* parameter)->EncodeThreadParameter_*
		{
			...
			parameter->code = CmpareMode::getTextFileEncodeType(parameter->filepath,1000, &bytes);

			//如果还是失败，再比较一下utf8 和 gbk 拉倒。如果没错，则就认定编码。这里反正不考虑速度，后续还可以增加一些人工探测的过程。
			if (parameter->code == CODE_ID::UNKOWN && bytes.size() > 0)
			{
				QString outText;
				bool codeSucess = Encode::tranStrToUNICODE(CODE_ID::UTF8_NOBOM, bytes.data(), bytes.size(), outText);
				if (codeSucess)
				{
					parameter->code = CODE_ID::UTF8_NOBOM;
				}
				else
				{
					codeSucess = Encode::tranStrToUNICODE(CODE_ID::GBK, bytes.data(), bytes.size(), outText);
					if (codeSucess)
					{
						parameter->code = CODE_ID::GBK;
					}
				}
...
static qint64 MAX_FILE_LIMIT = 1024 * 1024 * 10;

CODE_ID EncodeConvert::convertFileToCode(QString& filePath, CODE_ID srcCode, CODE_ID dstCode, int &error)
{
	...
	if (dstCode != UNKOWN)
	{
		QTextCodec* pTranCode = getTextCodeByCode(dstCode);
		...
		if (textOut.length() > 0)
		{
			//保存时注意编码问题。这个tolocal已经带了字符BOM头了。只要UTF8_BOM不会带。所有上面BOM UTF8要单独写头。
			file.write(pTranCode->fromUnicode(textOut));
		}
	}
...
//注意要与EncodeConvert中的下拉combox框中的列表对应起来。
	...
	return commitTask([=](EncodeThreadParameter_* parameter)->EncodeThreadParameter_*
		{
			...
			{
				parameter->code = convertFileToCode(parameter->filepath, srcCode, dstCode, parameter->error);
			}
			else
			{
...
void EncodeConvert::scanFileCode()
{
	...
	int scanMode = ui.extComboBox->currentIndex();
	...
	for (QList<fileAttriNode>::iterator iter = m_fileAttris.begin(); iter != m_fileAttris.end(); ++iter)
	{
		...
		//20230304 编码转换这里，不能仅仅只识别已知后缀文件，要识别所有文件
		//20231014 提供选项。根据不同的扫描模式，识别不同后缀的文件。
		else if ((iter->type == RC_FILE) && isSupportExt(scanMode,CompareDirs::fileSuffix(iter->relativePath)))
		{
	...
	while (m_finishCmpFileNums < m_commitCmpFileNums)
	{
		...
		//每更新一下
		if (curProcessRatio - finishProcessRatio >= 2)
		{
...
void EncodeConvert::slot_scanFileCode()
{
	...
	if (result != nullptr)
	{
		...
		if (result->error == 0 && result->code != CODE_ID::UNKOWN)
		{
			result->item->setData(0, ITEM_CAN_TRANS, 1);
		}
		else
		{
			result->item->setData(0, ITEM_CAN_TRANS, -1);
...
void EncodeConvert::slot_startConvert()
{
	if (m_workStatus != 0)
	{
		ui.logTextBrowser->append(tr("busying, please wait for load scan finish !"));
	...
	for (QList<fileAttriNode>::iterator iter = m_fileAttris.begin(); iter != m_fileAttris.end(); ++iter)
	{
		if ((iter->type == RC_FILE) && !iter->selfItem->isHidden() && isSupportExt(extComboBoxIndex, CompareDirs::fileSuffix(iter->relativePath)))
		{
	...
	if (m_transFailNums == 0)
	{
		ui.logTextBrowser->append(tr("total file %1, success %2, finish 100% .").arg(m_commitCmpFileNums).arg(m_finishCmpFileNums));
	}
	else
	{
		//有转换错误文件
		ui.logTextBrowser->append(tr("total file %1, success %2, failed %3 .").arg(m_commitCmpFileNums).arg(m_commitCmpFileNums - m_transFailNums).arg(m_transFailNums));
	}

	if (m_transFailLargeNums > 0)
	{
		//有一个大文件无法转换，请使用大文本处理器装换。
		//There is a large file that cannot be converted. Please use a large text processor to replace it.
		ui.logTextBrowser->append(tr("there is %1 large file that cannot be converted. Please use a large text processor to replace it .").arg(m_transFailLargeNums));
	}
...
void EncodeConvert::slot_convertFileFinish()
{
	...
	if (result != nullptr)
	{
		...
		{
			++m_transFailNums;
			++m_transFailLargeNums;
			result->item->setText(4, tr("convert fail [Exceeding size]"));
			ui.logTextBrowser->append(tr("file %1 convert failed, too large to convert, Please use a large text processor .").arg(result->item->data(0, Qt::ToolTipRole).toString()));
		}
		else 
		{
			++m_transFailNums;
